...
export const createWorkspaceStore = (
  ...
  const saveNow = async () => {
    ...
    saveTimer = undefined
    try { await api.saveSession(snapshot(store.getState())) } catch { /* non-fatal */ }
  }
  const scheduleSave = () => {
    if (disposed || store.getState().hydrationState !== 'ready') return
    if (saveTimer) clearTimeout(saveTimer)
  ...
  const closeIds = async (ids: Set<string>) => {
    ...
    await Promise.all([...new Set(removed.map((tab) => tab.fileId))]
      .filter((fileId) => fileId && !remainingIds.has(fileId))
      .map(async (fileId) => { try { await api.closeFile(fileId) } catch { /* non-fatal */ } }))
  }
  ...
  store = createStore<WorkspaceState>((set, get) => ({
    ...
    async openPaths(input) {
      const paths = [...new Set(input.map((path) => path.trim()).filter(Boolean))]
      if (!paths.length) return
      set({ opening: get().opening + paths.length })
      let outcomes: OpenFileOutcome[]
      try { outcomes = await api.openFiles(paths) } catch (error) {
        const appError: AppError = typeof error === 'object' && error !== null && 'code' in error
          ? error as AppError : { code: 'INTERNAL_ERROR', message: 'An internal error occurred', detail: null }
        set((state) => ({ opening: Math.max(0, state.opening - paths.length), pathErrors: { ...state.pathErrors, ...Object.fromEntries(paths.map((path) => [path, appError])) } }))
        return
      }
      let tabs = [...get().tabs]
      ...
      paths.forEach((path, index) => {
        const outcome = outcomes[index] ?? { ok: false, error: { code: 'INTERNAL_ERROR', message: 'An internal error occurred', detail: null } } as const
        if (!outcome.ok) { errors[path] = outcome.error; return }
        delete errors[path]
        ...
        const restored = tabs.find((tab) => tab.path === path && tab.status === 'unavailable')
        if (restored) {
          tabs = tabs.map((tab) => tab.id === restored.id ? { ...tab, fileId: outcome.metadata.fileId, path: outcome.metadata.path, metadata: outcome.metadata, status: 'ready' } : tab)
          active = restored.id
        } else {
          const id = tabId()
      ...
      set((state) => ({ tabs, activeTabId: active, pathErrors: errors, opening: Math.max(0, state.opening - paths.length) }))
      scheduleSave()
    },
    async hydrate() {
      ...
      hydrating = (async () => {
        try {
          ...
          if (available.length) {
            let outcomes: OpenFileOutcome[]
            try { outcomes = await api.openFiles(available.map((tab) => tab.path)) } catch { outcomes = [] }
            const byId = new Map(available.map((tab, index) => [tab.id, outcomes[index]]))
            set((state) => ({ tabs: state.tabs.map((tab) => {
              const outcome = byId.get(tab.id)
              if (!outcome) return tab.status === 'loading' ? { ...tab, status: 'error' } : tab
              return outcome.ok ? { ...tab, fileId: outcome.metadata.fileId, path: outcome.metadata.path, metadata: outcome.metadata, status: 'ready' } : { ...tab, status: 'error' }
            }) }))
          }
