...
test('a version change updates generated manifests without changing dependencies and is idempotent', () => {
  ...
  try {
    ...
    assert.match(read('src-tauri/Cargo.lock'), /name = "example"\nversion = "1.0.0"/)
    assert.match(read('src-tauri/Cargo.lock'), /name = "parquet-viewer"\nversion = "2.3.4"/)
